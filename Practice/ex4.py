...
def is_value_present(dictionary,value):
    # How can a check if the value is present in the dict object?
    # How can you find all values in dictionary object?
    # Values()
    return value in dictionary.values()

def is_key_present(dictionary,key):
    # I would get all the key in dictionary and check if the key is one of them.
...
# class def while for if return s
